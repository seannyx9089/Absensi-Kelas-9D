...
const studentsSeed: Student[] = [
  { id: 1, initials: "AF", name: "Ahmad Faisal Rizki", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 2, initials: "AS", name: "Aisha Shakira Gustiar", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 3, initials: "AA", name: "Akmal Agung Asyhari", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 4, initials: "AA", name: "Ayesha Anervian", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 5, initials: "DI", name: "Desvita Indriyani", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 6, initials: "DN", name: "Dewi Novianti", className: "IX D", status: "Izin", note: "Menunggu verifikasi" },
  { id: 7, initials: "EE", name: "Eliza Eliana", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 8, initials: "ES", name: "Eneng Sadira Aulia", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 9, initials: "IM", name: "Ilham Maulana", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 10, initials: "IA", name: "Irma Agustina", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 11, initials: "LS", name: "Laila Shafira Zahra", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 12, initials: "MD", name: "M Daffa Al-Goffar", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 13, initials: "MA", name: "M. Aldan Febrian", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 14, initials: "MM", name: "M. Marlian Maulana", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 15, initials: "MT", name: "M. Tegar Ramadhan", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 16, initials: "MJ", name: "Moch. Jildan Apriliandi", className: "IX D", status: "Telat", note: "Masuk 07.18" },
  { id: 17, initials: "MA", name: "Muhamad Abibi Arahman", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 18, initials: "MS", name: "Muhamad Sidik", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 19, initials: "MS", name: "Muhamad Syaripatul Akbar", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 20, initials: "MH", name: "Muhammad Haikal Aditya", className: "IX D", status: "Alpa", note: "Tidak ada keterangan" },
  { id: 21, initials: "MI", name: "Muhammad Ilyas", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 22, initials: "MR", name: "Muhammad Reza Setiawan", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 23, initials: "NS", name: "Nabil Saepul Anwar", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 24, initials: "NR", name: "Nadira Raeesa Mahadillan", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 25, initials: "RU", name: "Rahmi Ulfa Munawaroh", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 26, initials: "R", name: "Ramdani", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 27, initials: "RS", name: "Randiansa Saputra", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 28, initials: "RN", name: "Rifda Nurfadilah", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 29, initials: "RA", name: "Rizki Aprilliyo", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 30, initials: "RA", name: "Rizky Anugrahwan", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 31, initials: "SA", name: "Silvi Aulia", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 32, initials: "SH", name: "Siti Hardiyanti", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 33, initials: "SJ", name: "Siti Juliani", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 34, initials: "SM", name: "Siti Maulida", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 35, initials: "SS", name: "Siti Sarah Rahmawati", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 36, initials: "SZ", name: "Siti Zulfah Zakiah", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 37, initials: "SS", name: "Siva Sri Rahayu", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 38, initials: "TA", name: "Tira Aprilia", className: "IX D", status: "Hadir", note: "Belum dibuka" },
  { id: 39, initials: "YS", name: "Yasmin Syafka Galbina", className: "IX D", status: "Hadir", note: "Belum dibuka" },
];
  ...
  return (
    <div className={`flex items-center gap-3 ${compact ? "scale-90 origin-left" : ""}`}>
      <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white shadow-lg shadow-[#0A2540]/15 ring-1 ring-[#0A2540]/10">
        <img src="/manus-storage/BlackYellowWhiteY2kModernStarFashionLogo_20260909_201337_0000_3a002aef.png" alt="Logo Kelas IX D" className="h-full w-full object-cover" />
      </div>
      <div>
...
function Home() {
  ...
  const updateStatus = (id: number, status: AttendanceStatus) => {
    setStudents((current) => current.map((student) => student.id === id ? { ...student, status, note: status === "Hadir" ? "Ditandai oleh guru" : status === "Izin" ? "Menunggu verifikasi" : status === "Alpa" ? "Tidak ada keterangan" : "Ditandai telat" } : student));
    setNotice(`Status ${status.toLowerCase()} berhasil diperbarui.`);
  };
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#F5F0E6] text-[#243447]">
      ...
      {showLogin && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0A2540]/45 px-4 backdrop-blur-sm">
        <div className="w-full max-w-md rounded-[28px] bg-[#F5F0E6] p-6 shadow-2xl sm:p-8">
          ...
          <div className="mt-4 rounded-xl bg-white/70 px-3 py-2 text-xs leading-5 text-[#74808b]">Demo akses: <strong className="text-[#0A2540]">admin</strong> / <strong className="text-[#0A2540]">smp123</strong></div>
          <button onClick={() => { if (loginUser === "admin" && loginPass === "smp123") { setLoggedIn(true); setShowLogin(false); setMode("teacher"); setNotice("Login guru berhasil."); window.setTimeout(() => document.getElementById("dashboard")?.scrollIntoView({ behavior: "smooth" }), 20); } else { setNotice("Username atau password admin belum sesuai."); } }} className="mt-6 w-full rounded-xl bg-[#0A2540] px-4 py-3.5 text-sm font-extrabold text-[#F5F0E6]">Masuk ke dashboard <ArrowRight className="ml-2 inline h-4 w-4" /></button>
        </div>
      </div>}
      ...
      <main>
        ...
        <section id="dashboard" className={`border-y border-[#0A2540]/8 bg-white/45 ${mode === "teacher" ? "" : ""}`}><div className="section-shell py-20"><div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><SectionLabel eyebrow="Ruang guru" title="Kehadiran hari ini" copy="Absensi belum dibuka · IX D" /><div className="flex gap-2"><button onClick={() => setNotice("Rekap berhasil disiapkan untuk diunduh.")} className="rounded-xl border border-[#0A2540]/12 bg-white px-4 py-2.5 text-xs font-extrabold text-[#0A2540] transition hover:bg-[#F5F0E6]"><Download className="mr-2 inline h-4 w-4" />Export rekap</button><button onClick={() => setNotice("Tanggal absensi siap dipilih.")} className="rounded-xl bg-[#0A2540] px-4 py-2.5 text-xs font-extrabold text-[#F5F0E6]"><CalendarDays className="mr-2 inline h-4 w-4" />Pilih tanggal</button></div></div><div className="mb-5 grid gap-3 sm:grid-cols-4"><MiniStat label="Total siswa" value="39" icon={<Users />} color="#0A2540" /><MiniStat label="Hadir" value={String(students.filter(s => s.status === "Hadir").length)} icon={<CheckCircle2 />} color="#1f8f64" /><MiniStat label="Menunggu" value={String(students.filter(s => s.status === "Izin").length)} icon={<Clock3 />} color="#c98b21" /><MiniStat label="Belum masuk" value={String(students.filter(s => s.status === "Alpa").length)} icon={<XCircle />} color="#c84d4d" /></div><div className="overflow-hidden rounded-[24px] border border-[#0A2540]/8 bg-white card-shadow"><div className="flex flex-col gap-3 border-b border-[#0A2540]/8 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2 text-sm font-extrabold text-[#0A2540]"><span className="h-2 w-2 rounded-full bg-[#1f8f64]" />Daftar siswa</div><div className="flex gap-2 text-xs font-bold text-[#74808b]"><button className="rounded-lg bg-[#F5F0E6] px-3 py-2 text-[#0A2540]">Semua (39)</button><button className="rounded-lg px-3 py-2 hover:bg-[#F5F0E6]">Perlu tinjauan (2)</button></div></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead className="bg-[#fbf9f5] text-[11px] uppercase tracking-[.14em] text-[#74808b]"><tr><th className="px-5 py-4 font-extrabold">Siswa</th><th className="px-5 py-4 font-extrabold">Status hari ini</th><th className="px-5 py-4 font-extrabold">Catatan</th><th className="px-5 py-4 text-right font-extrabold">Tindakan</th></tr></thead><tbody>{students.map((student) => <tr key={student.id} className="border-t border-[#0A2540]/6 transition hover:bg-[#fbf9f5]"><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e7edf1] text-xs font-extrabold text-[#0A2540]">{student.initials}</span><div><div className="text-sm font-extrabold text-[#0A2540]">{student.name}</div><div className="text-xs text-[#74808b]">{student.className}</div></div></div></td><td className="px-5 py-4"><StatusBadge status={student.status} /></td><td className="px-5 py-4 text-sm text-[#667788]">{student.note}</td><td className="px-5 py-4 text-right">{student.status === "Izin" ? <div className="flex justify-end gap-2"><button onClick={() => updateStatus(student.id, "Izin")} className="rounded-lg bg-[#e2f3eb] p-2 text-[#1f8f64]" aria-label="Setujui izin"><Check className="h-4 w-4" /></button><button onClick={() => updateStatus(student.id, "Alpa")} className="rounded-lg bg-[#f9e1df] p-2 text-[#c84d4d]" aria-label="Tolak izin"><X className="h-4 w-4" /></button></div> : <select aria-label={`Ubah status ${student.name}`} value={student.status} onChange={(e) => updateStatus(student.id, e.target.value as AttendanceStatus)} className="rounded-lg border border-[#0A2540]/10 bg-[#fbf9f5] px-2 py-2 text-xs font-bold text-[#0A2540]"><option>Hadir</option><option>Izin</option><option>Telat</option><option>Alpa</option></select>}</td></tr>)}</tbody></table></div></div></div></section>
...
function ParentForm({ onSent }: { onSent: () => void }) {
  const [student, setStudent] = useState("Dewi Novianti"); const [kind, setKind] = useState("Sakit"); const [studentSearch, setStudentSearch] = useState(""); const [studentMenuOpen, setStudentMenuOpen] = useState(false); const [recording, setRecording] = useState(false); const [seconds, setSeconds] = useState(0); const [evidence, setEvidence] = useState<File | null>(null); const [sent, setSent] = useState(false); const timer = useRef<number | null>(null);
  const startRecording = () => { setRecording(true); setSeconds(0); timer.current = window.setInterval(() => setSeconds((value) => value + 1), 1000); };
  const stopRecording = () => { setRecording(false); if (timer.current) window.clearInterval(timer.current); };
  const send = () => { setSent(true); stopRecording(); onSent(); };
  return <div className="rounded-[28px] border border-[#0A2540]/8 bg-white p-5 card-shadow sm:p-7"><div className="mb-6 flex items-start justify-between"><div><div className="font-display text-xl font-extrabold text-[#0A2540]">Formulir izin</div><div className="mt-1 text-sm text-[#74808b]">Menunggu pembukaan absensi guru</div></div><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#e2f3eb] text-[#1f8f64]"><FileText className="h-5 w-5" /></div></div>{sent ? <div className="rounded-2xl bg-[#e2f3eb] p-5"><CheckCircle2 className="h-8 w-8 text-[#1f8f64]" /><div className="mt-3 font-display text-lg font-extrabold text-[#176746]">Izin terkirim.</div><p className="mt-1 text-sm leading-6 text-[#4b806b]">Guru akan menerima notifikasi dan mengabari setelah verifikasi selesai.</p><button onClick={() => setSent(false)} className="mt-4 text-sm font-extrabold text-[#176746] underline">Kirim izin lain</button></div> : <><label className="mb-2 block text-xs font-extrabold uppercase tracking-[.13em] text-[#74808b]">Pilih siswa <span className="font-medium normal-case tracking-normal text-[#1f8f64]">· 39 siswa terdaftar</span></label><div className="relative"><button type="button" onClick={() => setStudentMenuOpen((open) => !open)} className="flex w-full items-center gap-3 rounded-xl border border-[#0A2540]/12 bg-[#fbf9f5] px-3 py-2.5 text-left outline-none transition hover:border-[#1f8f64]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e7edf1] text-xs font-extrabold text-[#0A2540]">{student.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span><span className="flex-1"><span className="block text-sm font-extrabold text-[#0A2540]">{student}</span><span className="mt-0.5 block text-[11px] text-[#74808b]">IX D · ketuk untuk mengganti</span></span><ChevronDown className={`h-4 w-4 text-[#74808b] transition ${studentMenuOpen ? "rotate-180" : ""}`} /></button>{studentMenuOpen && <div className="absolute left-0 right-0 z-20 mt-2 overflow-hidden rounded-2xl border border-[#0A2540]/10 bg-white p-2 shadow-2xl"><div className="relative"><Users className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[#74808b]" /><input autoFocus value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)} placeholder="Cari nama siswa..." className="w-full rounded-xl bg-[#F5F0E6] py-2.5 pl-9 pr-3 text-sm font-semibold text-[#0A2540] outline-none" /></div><div className="mt-2 max-h-52 space-y-1 overflow-y-auto">{studentNames.filter((name) => name.toLowerCase().includes(studentSearch.toLowerCase())).map((name) => <button type="button" key={name} onClick={() => { setStudent(name); setStudentMenuOpen(false); setStudentSearch(""); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition hover:bg-[#F5F0E6] ${name === student ? "bg-[#e2f3eb]" : ""}`}><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#e7edf1] text-[10px] font-extrabold text-[#0A2540]">{name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span><span className="text-sm font-bold text-[#0A2540]">{name}</span>{name === student && <Check className="ml-auto h-4 w-4 text-[#1f8f64]" />}</button>)} </div><div className="border-t border-[#0A2540]/8 px-2 pb-1 pt-2 text-[11px] font-semibold text-[#74808b]">Daftar lengkap IX D · SMP PGRI 1 CIDAHU</div></div>}</div><label className="mb-2 mt-5 block text-xs font-extrabold uppercase tracking-[.13em] text-[#74808b]">Jenis keterangan</label><div className="grid grid-cols-2 gap-2"><button onClick={() => setKind("Sakit")} className={`rounded-xl border px-4 py-3 text-sm font-extrabold transition ${kind === "Sakit" ? "border-[#0A2540] bg-[#0A2540] text-[#F5F0E6]" : "border-[#0A2540]/10 bg-[#fbf9f5] text-[#74808b]"}`}>Sakit</button><button onClick={() => setKind("Keperluan keluarga")} className={`rounded-xl border px-4 py-3 text-sm font-extrabold transition ${kind === "Keperluan keluarga" ? "border-[#0A2540] bg-[#0A2540] text-[#F5F0E6]" : "border-[#0A2540]/10 bg-[#fbf9f5] text-[#74808b]"}`}>Keperluan keluarga</button></div><div className="mt-5 rounded-2xl border border-dashed border-[#1f8f64]/35 bg-[#f5fbf7] p-4"><div className="flex items-center gap-3"><button onClick={recording ? stopRecording : startRecording} className={`flex h-11 w-11 items-center justify-center rounded-xl ${recording ? "bg-[#c84d4d] text-white" : "bg-[#1f8f64] text-white"}`}>{recording ? <span className="h-3 w-3 rounded-sm bg-white" /> : <Mic className="h-5 w-5" />}</button><div className="flex-1"><div className="text-sm font-extrabold text-[#0A2540]">{recording ? `Merekam suara · 00:${String(seconds).padStart(2, "0")}` : "Rekam VN untuk guru"}</div><div className="mt-1 text-xs text-[#74808b]">{recording ? "Tekan tombol untuk berhenti" : "Opsional · maksimal 60 detik"}</div></div>{recording && <div className="flex items-end gap-0.5">{[3,8,5,11,7,14,6,10].map((height, index) => <span key={index} className="w-1 rounded-full bg-[#1f8f64]" style={{ height }} />)}</div>}</div></div><label className="mt-4 flex cursor-pointer items-center gap-3 rounded-2xl border border-[#0A2540]/10 bg-[#fbf9f5] p-4 transition hover:bg-[#F5F0E6]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-[#0A2540]"><Paperclip className="h-4 w-4" /></span><span className="flex-1"><span className="block text-sm font-extrabold text-[#0A2540]">Lampirkan bukti <span className="font-medium text-[#74808b]">(opsional)</span></span><span className="mt-1 block text-xs text-[#74808b]">Foto atau PDF · maks. 5 MB</span></span><input type="file" accept="image/*,.pdf" className="hidden" onChange={(e) => setEvidence(e.target.files?.[0] ?? null)} />{evidence ? <Check className="h-5 w-5 text-[#1f8f64]" /> : <Plus className="h-5 w-5 text-[#74808b]" />}</label>{evidence && <div className="mt-2 truncate text-xs font-bold text-[#1f8f64]">Terpilih: {evidence.name}</div>}<button onClick={send} className="mt-6 w-full rounded-xl bg-[#0A2540] px-4 py-3.5 text-sm font-extrabold text-[#F5F0E6] shadow-lg shadow-[#0A2540]/15 transition hover:bg-[#143b5d]">Kirim izin <ArrowRight className="ml-2 inline h-4 w-4" /></button></>}</div>;
}
